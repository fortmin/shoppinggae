...
public class Clientes {
     ...
     */
    public Mensaje getUbicacionCliente(EntityManager mgr, String usuario) {
	Mensaje result = null;
	Cliente cliente = mgr.find(Cliente.class, usuario);
	if (cliente != null) {
	    if (cliente.isPresente())
		if (cliente.getUbicacionTemp() != null)
		    result = new Mensaje("GetUbicacionCliente", "OK",
			    cliente.getUbicacionTemp());
		else
		    result = new Mensaje("GetUbicacionCliente", usuario
			    + "::SIN_UBICACION");
	    else
		result = new Mensaje("GetUbicacionCliente", usuario
			+ "::CLIENTE_AUSENTE");
	...
	while (ilamigos.hasNext()) {
	    ...
	    Mensaje mens = this.getUbicacionCliente(mgr, amigo);
	    if (mens.getMensaje().equals("OK"))
		msgendp.enviarMensaje(amigo + " esta en el " + mens.getTexto()
			+ "del Shopping", usuario);
	    else
		msgendp.enviarMensaje(amigo + " esta en el Shopping", usuario);
	}
    }
     ...
     * Quita puntos al cliente, utilizada para la transferencia de puntos
     */
    public Mensaje canjearPuntos(EntityManager mgr, String usuario,
	    String nompaq) {
	Mensaje result = null;
	EntityTransaction txn = mgr.getTransaction();
	txn.begin();
	...
	if (cliente != null) {
	    ...
	    Paquete paquete = paqs.getPaquete(mgr, nompaq);
	    if (paquete != null) {
		int ptospaq = paquete.getPuntos();
		if (cliente.getPuntaje() >= ptospaq) {
		    cliente.setPuntaje(cliente.getPuntaje() - ptospaq);
		    mgr.persist(cliente);
		    txn.commit();
		    result = new Mensaje("QuitarPuntos", "OK");
		} else {
		    txn.rollback();
		    result = new Mensaje("CanjearPuntos", usuario
			    + "::SALDO_INSUFICIENTE");
		}
	    } else {
		txn.rollback();
     ...
     * Lista las compras del cliente
     */
    public LinkedList<ComprasVO> getComprasPendienteEntrega(EntityManager mgr) {
	LinkedList<ComprasVO> comprasvo = new LinkedList<ComprasVO>();
	EntityTransaction txn = mgr.getTransaction();
	txn.begin();
	Compras comps = new Compras();
	List<Compra> compras = comps.getComprasPendienteEntrega(mgr);
	if (compras != null) {
	    ComprasVO compvo;
	    Compra comp;
	    Iterator<Compra> icompras = compras.iterator();
